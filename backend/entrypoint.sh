...
if [ "$USE_INTERNAL_DB" = "true" ]; then
  ...
  # Create user if not exist (write SQL to temp file with variables expanded)
...
EOF
  ...
  # Make user the owner of the database and public schema, and grant privileges
...
# Only run fix_permissions.py for external DB setups
if [ "$USE_INTERNAL_DB" != "true" ]; then
...
# Start supervisor (nginx, gunicorn, etc.)
